...
app = Flask(__name__)   # Default: looks for templates in templates/ folder
...
def get_stats():
    ...
    return dict(
        members  = max(0, User.query.count() - 1),
...
def signup_page():
    ...
    if request.method == 'POST':
        ...
        send_email(email, 'Welcome to Inner Compass Project! 🧭',
            ...
            f'<p><a href="https://innercompassproject.up.railway.app/dashboard">Visit your dashboard →</a></p>'
            f'<p style="color:#5a7080;font-size:.9rem">Inner Compass Project · 08145739207</p></div>')
        notify_club(f'New Member: {name}',
            f'<p><b>Name:</b> {name}<br><b>Email:</b> {email}</p>')
...
def reset_admin():
    ...
    u     = User.query.filter_by(email=email).first()
    if u:
        u.password_hash = h; u.role = 'admin'; db.session.commit(); msg = 'Password reset!'
    else:
        db.session.add(User(username='Mustapha Abdulrasak', email=email,
                            password_hash=h, role='admin'))
        db.session.commit(); msg = 'Admin created!'
    return (f'<div style="font-family:sans-serif;max-width:460px;margin:80px auto;'
            f'text-align:center;padding:2rem;border:2px solid #4ead7d;border-radius:16px">'
            f'<h2 style="color:#2d9b6a">✅ {msg}</h2>'
            f'<p><b>Email:</b> {email}</p><p><b>Password:</b> {pw}</p><br>'
            f'<a href="/icp-admin-login" style="background:#4ead7d;color:white;padding:.75rem 2rem;'
            f'border-radius:100px;text-decoration:none;font-weight:500">Go to Admin Login →</a><br><br>'
            f'<p style="color:#e74c3c;font-size:.82rem">⚠️ Delete this route after logging in!</p></div>')
...
def api_contact():
    ...
    name, email, subject, message = (
        d.get('name','').strip(), d.get('email','').strip(),
    ...
    notify_club(f'Contact: {subject}',
        ...
        f'<p style="background:#f5f5f5;padding:1rem;border-radius:8px">{message}</p>'
        f'<p><a href="mailto:{email}">Reply to {name}</a></p>')
...
def api_subscribe():
    ...
    send_email(email, "You're subscribed to Inner Compass Project! 🌿",
        '<div style="font-family:sans-serif"><h2 style="color:#2d9b6a">You\'re in! 🎉</h2>'
        '<p>Monthly mental health insights coming your way.</p>'
        '<p><a href="https://innercompassproject.up.railway.app/resources">Browse Resources →</a></p>'
        '<p style="color:#5a7080;font-size:.85rem">Inner Compass Project · Ilorin, Kwara State</p></div>')
    notify_club('New Newsletter Subscriber',
        f'<p>New: <b>{email}</b> — {datetime.utcnow().strftime("%b %d, %Y")}</p>')
...
def api_mood():
    ...
    db.session.commit()
    r = {'happy': "That's wonderful! Keep spreading that energy. 🌟",
...
def e500(e):
    return (f'<h2 style="font-family:sans-serif;color:#e74c3c">Server Error</h2>'
            f'<pre>{e}</pre><a href="/">← Home</a>'), 500
