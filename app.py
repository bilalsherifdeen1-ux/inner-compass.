...
@app.route('/')
def home():
    return render_template('index.html', stats=get_stats(), current_user=current_user)
...
def signup():
    ...
    if request.method == 'POST':
        ...
        if not all([username, email, password]):
            return (jsonify({"success": False, "error": "All fields are required."}), 400) if request.is_json else redirect(url_for('signup'))
        if len(password) < 8:
            return (jsonify({"success": False, "error": "Password must be at least 8 characters."}), 400) if request.is_json else redirect(url_for('signup'))
        if User.query.filter_by(email=email).first():
            return (jsonify({"success": False, "error": "Email already registered."}), 409) if request.is_json else redirect(url_for('login'))
...
def login():
    ...
    if request.method == 'POST':
        ...
        if user and check_password_hash(user.password_hash, password):
            ...
            return (jsonify({"success": False, "error": "Invalid email or password."}), 401) if request.is_json else redirect(url_for('login'))
...
def dashboard():
    ...
    return render_template('dashboard.html', user=current_user,
                           ...
                           stats=get_stats(), mood_logs=mood_logs)
...
# ── Cloudinary Upload (keep existing) ────────────────────────────────────────
    ...
    if not User.query.filter_by(role='admin').first():
        admin_user = User(
            ...
            password_hash = generate_password_hash(
                os.environ.get('ADMIN_PASSWORD', 'ICP@Admin2025!'), method='pbkdf2:sha256'),
            role = 'admin'
        )
